...
@cython.boundscheck(False)
@cython.wraparound(False)
def watershed_raveled(cnp.float64_t[::1] image1,
                      cnp.float64_t[::1] image2,
                      cnp.float64_t[::1] image3,
    ...
    with nogil:
        ...
        while hp.items > 0:
            ...
            if compact or wsl:
                ...
                distancemap[elem.index] = elem.value 
            ...
            for i in range(nneighbors):
                ...
                if method:
                    diff1 = image1[neighbor_index] - image1[elem.index]
                    diff2 = image2[neighbor_index] - image2[elem.index]
                    diff3 = image3[neighbor_index] - image3[elem.index]
                    new_elem.value = elem.value + sqrt(diff1**2+diff2**2+diff3**2)
                else:
                    new_elem.value = image1[neighbor_index]
                ...
                if compact:
                    ...
                    distancemap[neighbor_index] = new_elem.value
                new_elem.age = age
